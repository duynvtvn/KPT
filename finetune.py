...
def main(args, model: Optional[PrefixDialogModule] = None) -> PrefixDialogModule:

    Path(args.output_dir).mkdir(exist_ok=True)
    check_output_dir(args, expected_items=3)

    # Check if CUDA_VISIBLE_DEVICES is set, and handle it if not
    if "CUDA_VISIBLE_DEVICES" in os.environ:
        args.gpus = len(os.environ["CUDA_VISIBLE_DEVICES"].split(","))
    else:
        # If CUDA_VISIBLE_DEVICES is not set, default to using all available GPUs
